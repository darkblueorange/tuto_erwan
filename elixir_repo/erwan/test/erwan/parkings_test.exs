defmodule Erwan.ParkingsTest do
  ...
  describe "rochelle_parkings" do
    ...
    @invalid_attrs %{nb_places: nil, nb_autopartage: nil, nb_velo: nil, nb_velo_dispo: nil, nb_2r_el_dispo: nil, parking_string_id: nil, ylat: nil, nb_2_rm: nil, nb_2_rm_dispo: nil, nb_2r_el: nil, xlong: nil, nom: nil, nb_voitures_electriques: nil, nb_places_disponibles: nil, nb_pr_dispo: nil, nb_pmr: nil, coord_y: nil, coord_x: nil, nb_voitures_electriques_dispo: nil, total_count: nil, full_binary_text: nil, nb_pmr_dispo: nil, nb_pr: nil, other_id: nil, nb_autopartage_dispo: nil, date_comptage: nil}
    ...
    test "create_parking_rochelle/1 with valid data creates a parking_rochelle" do
      valid_attrs = %{nb_places: 42, nb_autopartage: 42, nb_velo: 42, nb_velo_dispo: 42, nb_2r_el_dispo: 42, parking_string_id: "some parking_string_id", ylat: 120.5, nb_2_rm: 42, nb_2_rm_dispo: 42, nb_2r_el: 42, xlong: 120.5, nom: "some nom", nb_voitures_electriques: 42, nb_places_disponibles: 42, nb_pr_dispo: 42, nb_pmr: 42, coord_y: 120.5, coord_x: 120.5, nb_voitures_electriques_dispo: 42, total_count: 42, full_binary_text: "some full_binary_text", nb_pmr_dispo: 42, nb_pr: 42, other_id: 42, nb_autopartage_dispo: 42, date_comptage: ~N[2023-09-05 23:03:00]}

      assert {:ok, %ParkingRochelle{} = parking_rochelle} = Parkings.create_parking_rochelle(valid_attrs)
    ...
    test "update_parking_rochelle/2 with valid data updates the parking_rochelle" do
      parking_rochelle = parking_rochelle_fixture()
      update_attrs = %{nb_places: 43, nb_autopartage: 43, nb_velo: 43, nb_velo_dispo: 43, nb_2r_el_dispo: 43, parking_string_id: "some updated parking_string_id", ylat: 456.7, nb_2_rm: 43, nb_2_rm_dispo: 43, nb_2r_el: 43, xlong: 456.7, nom: "some updated nom", nb_voitures_electriques: 43, nb_places_disponibles: 43, nb_pr_dispo: 43, nb_pmr: 43, coord_y: 456.7, coord_x: 456.7, nb_voitures_electriques_dispo: 43, total_count: 43, full_binary_text: "some updated full_binary_text", nb_pmr_dispo: 43, nb_pr: 43, other_id: 43, nb_autopartage_dispo: 43, date_comptage: ~N[2023-09-06 23:03:00]}

      assert {:ok, %ParkingRochelle{} = parking_rochelle} = Parkings.update_parking_rochelle(parking_rochelle, update_attrs)
    ...
    test "update_parking_rochelle/2 with invalid data returns error changeset" do
      parking_rochelle = parking_rochelle_fixture()
      assert {:error, %Ecto.Changeset{}} = Parkings.update_parking_rochelle(parking_rochelle, @invalid_attrs)
    ...
    test "delete_parking_rochelle/1 deletes the parking_rochelle" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Parkings.get_parking_rochelle!(parking_rochelle.id) end
    end
